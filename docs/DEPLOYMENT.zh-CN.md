...
RemoteHubStudio 使用 .NET 10、Windows Forms、AntdUI 和 Windows API。默认发布 **Windows x86、x64、ARM64 独立部署 ZIP，以及一个 Windows 可移植 ZIP（AnyCPU DLL + EXE 启动器）**。ZIP 都可以完整解压到可写目录使用；程序的 `data` 目录存放用户数据，更新程序时应保留。外部远程客户端仍需单独安装。
...
| 可移植 / 不指定 RID | 支持，默认启用 | AnyCPU DLL + 原生 EXE，不附带运行时；可双击 `RemoteHubStudio.exe`（当前 Actions 为 x64），也可用 `dotnet RemoteHubStudio.dll` 或 `Start-RemoteHubStudio.cmd` 启动 |
...
可移植包的 EXE 有固定架构，需要安装对应架构的 .NET 10 **Windows Desktop Runtime**。使用 CMD 启动时，则由 `PATH` 中选中的 `dotnet` 决定进程架构，并需要该架构的 Desktop Runtime。普通 `.NET Runtime` 或 ASP.NET Core Runtime 不包含 WinForms。需要明确选架构时，使用相应架构的 `dotnet.exe` 完整路径运行 DLL。独立部署的用户通常直接选择对应 CPU 的 EXE 包更方便。

## 为什么 Visual Studio 的“可移植”发布也有 EXE

可移植发布仍可附带原生启动器（apphost）。之前的脚本显式设置 `UseAppHost=false`，所以只有 DLL 和 CMD；现在与 Visual Studio 的默认行为保持一致，使用 **空 `RuntimeIdentifier` + `UseAppHost=true` + `SelfContained=false` + `PlatformTarget=AnyCPU`**。

**AnyCPU 指的是 DLL，EXE 本身不是全架构通用的。**不指定 RID 时，SDK 默认按自身平台生成启动器；当前 Actions 的 x64 SDK 生成 x64 EXE，本地其他架构 SDK 的结果可能不同。脚本读取生成 EXE 的实际架构，写入 `PACKAGE-README.txt` 和 Release 下载表；每个包缺少 EXE 都会使打包失败。

在 Windows x86/ARM64 上需要对应架构的 EXE 时，选择 `win-x86` / `win-arm64` 包；也可以继续使用可移植包的 AnyCPU DLL，通过对应架构的 `dotnet` 启动。`Start-RemoteHubStudio.cmd` 保留作为 DLL 启动入口，不调用包内的 x64 EXE。
...
**All Releases (Manual)、Force Build and Release (Manual)、Nightly Release、Stable Release、Build Release Package** 的 **Run workflow** 都有 `deployment-mode` 参数：
...
手动总任务把参数传给两个发布子任务，再传给共用打包流程。`IncludePortable` 开启时，每种选择都额外生成一个可移植依赖框架包；不会生成重复的可移植包。独立的 **Daily Releases (Scheduled)** 入口每天北京时间 **00:00** 定时运行；定时和标签触发没有手动参数，自动使用仓库配置。
...
所有架构、部署模式的正式版和预览版 ZIP 都包含一个与压缩包同名（去掉 `.zip`）的顶级文件夹。直接解压到当前目录时，程序文件会集中在该文件夹内。解压后进入文件夹，再启动 EXE；可移植包也可使用 CMD。例如：
...
**普通发布保留已有正式版附件。**需要让新架构或部署模式应用到同一版本时，在默认分支运行 [Force Build and Release (Manual)](../.github/workflows/force-build.yml)，选择 `nightly`、`stable` 或 `both` 及部署模式。它重新构建当前源码、执行测试、替换对应 Release 的 ZIP 和校验和、清理过时的程序 ZIP，并更新构建说明和源码标签。`stable` 要求源码版本为纯 `X.Y.Z`；不可变 Release 无法替换。更新后的附件直接从 **Releases** 下载，同时保留 Actions Artifact 7 天。仅需构建产物时使用 **Build Release Package**。普通发布构建现有标签时，共用流程使用调用工作流对应版本的打包脚本及配置，应用代码仍来自选定的源码提交。
