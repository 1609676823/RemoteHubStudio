@{
    ...
    # No RID + UseAppHost=true: AnyCPU DLL + the SDK's native EXE (x64 on current Actions runners).
    # 与 VS“可移植”发布一致，保留 EXE；EXE 有固定架构，DLL 可通过其他架构的 dotnet 启动。
    # Always framework-dependent / 始终依赖框架。
