...
$checksumLines = @()
$tableRows = @()
$packageDirectoryName = 'RemoteHubStudio'
...
foreach ($package in $packages) {
    ...
    $archiveBaseName = "RemoteHubStudio-$ReleaseTag-$runtime-$mode"
    # Keep the download name descriptive and the extracted folder stable for in-place upgrades.
    # An isolated parent per target prevents different architectures or modes sharing output.
    # / 下载名称保留版本、架构和模式；包内统一为 RemoteHubStudio，方便覆盖升级。
    # / 每种组合使用独立的父目录，避免不同架构或部署模式的产物混用。
    $publishDirectory = Join-Path (Join-Path $scratchRoot $archiveBaseName) $packageDirectoryName
...
"@ | Set-Content -LiteralPath (Join-Path $publishDirectory 'PACKAGE-README.txt') -Encoding utf8NoBOM
    ...
    # Archive only the stable inner directory, keeping the per-target parent outside the ZIP.
    # / 只压缩固定名称的内层目录，不把用于隔离构建的父目录放入 ZIP。
...
Each ZIP contains one top-level folder named RemoteHubStudio. Open that folder to start the application.
每个 ZIP 内只有一个名为 RemoteHubStudio 的顶级文件夹；解压后进入该文件夹启动程序。
To upgrade, exit the application, replace the files in your local application folder, and preserve its data directory.
升级时请先退出程序，将新包中的文件覆盖到本地软件目录，并保留其中的 data 用户数据目录。
