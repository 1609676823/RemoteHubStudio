...
/// <summary>
/// Runs executable regression checks for core RemoteHubStudio behavior. / 为 RemoteHubStudio 核心行为运行可独立执行的回归检查。
/// </summary>
internal static class Program
{
    ...
    private static async Task<int> Main(string[] args)
    {
        try
        {
            ...
            {
                await ConnectionLaunchCancellationRegression.RunAsync();
                return 0;
            }
            if (args.FirstOrDefault() == "--connection-launch-ui-tests")
            {
                ConnectionLaunchUiRegression.Run();
                return 0;
            }
            if (args.FirstOrDefault() == "--todesk-command-probe") return ToDeskStartupRegression.RunProbe(args);
            if (args.FirstOrDefault() == "--todesk-window-probe") return ToDeskStartupRegression.RunWindowProbe(args);
            if (args.FirstOrDefault() == "--todesk-readiness-probe")
            {
                await ToDeskClientStartup.Shared.EnsureRunningAsync(args[1]);
                Console.WriteLine("TODESK_READINESS_OK");
                return 0;
            }
            if (args.FirstOrDefault() == "--todesk-startup-tests")
            {
                await ToDeskStartupRegression.RunAsync();
                return 0;
            }
            if (args.FirstOrDefault() == "--single-instance-probe") return SingleInstanceRegression.RunProbe(args);
            if (args.FirstOrDefault() == "--main-window-layout-probe")
            {
                System.Windows.Forms.Application.SetHighDpiMode(HighDpiMode.PerMonitorV2);
                System.Windows.Forms.Application.EnableVisualStyles();
                WindowPlacementStoreRegression.Run();
                MainWindowLayoutRegression.Run();
                return 0;
            }
    ...
    private static void TestMainResponsiveLayoutLogic()
    {
        ...
        MainResponsiveLayoutPlan highDpiPlan = MainResponsiveLayoutLogic.CreatePlan(1366F / 2F - 168F - 32F, highDpiClientHeight);
        ...
        MainResponsiveLayoutPlan scaledPlan = MainResponsiveLayoutLogic.CreatePlan(1024F / 1.5F - 168F - 32F, 768F / 1.5F);
        ...
        MainResponsiveLayoutPlan smallHighDpiPlan = MainResponsiveLayoutLogic.CreatePlan(496F - 168F - 32F, 348F);
        Assert(smallHighDpiPlan.UseToolbarOverflow && smallHighDpiPlan.ToolbarHeight == 104 &&
               smallHighDpiPlan.SearchWidth + smallHighDpiPlan.TypeFilterWidth + 16 <= 296 &&
               348 - 110 - smallHighDpiPlan.ToolbarHeight - smallHighDpiPlan.SecondaryToolbarHeight >= smallHighDpiPlan.MinimumTableHeight,
            "The 1024x728 working area at 200% cannot fit its filters and table beside expanded groups. / 1024x728 工作区 @ 200% 无法在展开分组旁容纳筛选器与表格。");

        MainResponsiveLayoutPlan narrowPlan = MainResponsiveLayoutLogic.CreatePlan(480F - 168F - 32F, 800F);
