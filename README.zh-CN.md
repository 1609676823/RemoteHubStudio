...
可从 [Releases](https://github.com/1609676823/RemoteHubStudio/releases) 下载正式版或每日预览版；带 `nightly` 的版本为预览版。普通用户选择对应 CPU 的 `win-x86`、`win-x64` 或 `win-arm64` **self-contained** ZIP，完整解压后运行 `RemoteHubStudio.exe`，无需另装 .NET。**framework-dependent** 包和 **win-portable** 可移植包需要 .NET 10 Windows Desktop Runtime；可移植包也包含 `RemoteHubStudio.exe`（当前 Actions 为 x64，需 x64 Desktop Runtime），并保留 `Start-RemoteHubStudio.cmd` / `dotnet RemoteHubStudio.dll`，可使用其他架构的 dotnet 和对应 Desktop Runtime 启动 AnyCPU DLL。详见 [发布目标与部署模式](docs/DEPLOYMENT.zh-CN.md)。
...
所有连接类型在启动期间都会显示“**停止连接**”按钮。点击后会取消当前启动/等待，恢复操作；真正退出应用时也会取消连接等待。超时覆盖参数生成、ToDesk 等客户端的就绪等待以及程序启动，默认 **15 秒**。已交接给外部客户端的会话由该客户端管理，此按钮用于停止尚未完成的启动过程。

启动新的应用进程前，可通过环境变量修改总超时（单位为整数秒，范围 `1–3600`；缺失、无效或越界时使用 15 秒）：
...
设置在应用启动时读取。若已有实例运行，请先退出该实例再重新启动。默认值、范围和解析规则集中在 `RemoteHubStudio/Infrastructure/Launch/ConnectionLaunchOptions.cs`，含修改说明。ToDesk 的 10 秒初始化缓冲与 3 秒稳定检查在 `ToDeskClientStartup.CreateDefault` 中调整，总等待仍受上述超时限制。

### ToDesk 说明

连接前会检查配置路径对应的当前 Windows 会话中的 ToDesk 客户端；未运行时先无参数打开程序。客户端启动满 10 秒后，还需所有窗口线程连续响应 3 秒，才发送连接命令；期间卡顿、进程或窗口更换会重新计算稳定时间。已有客户端会复用，等待过程不会阻塞主窗口，并显示自动等待提示；受上述总超时限制，恢复后自动继续，无需再次点击。停止、启动失败或一直无响应会结束等待，不发送连接命令。自定义 ToDesk 参数也经过相同的启动检查。窗口响应检查不等同于远端连接成功确认。
...
自动和手动使用两个独立入口：[Daily Releases (Scheduled)](.github/workflows/daily-release.yml) 是唯一定时入口，[All Releases (Manual)](.github/workflows/all-releases.yml) 用于手动运行。采用 GitHub 的[可复用工作流模式](https://docs.github.com/en/actions/how-tos/reuse-automations/reuse-workflows)：入口各自管理触发方式和参数，通过 `workflow_call` 共用构建与发布逻辑，自动和手动运行记录在 Actions 中分开展示。每个总任务都**并行**调用预览版和正式版子工作流，在同一次运行页面中可查看两个子任务及其编译、测试、发布步骤；两个子任务没有先后依赖，一个失败不会阻止另一个执行。所有子工作流仍支持独立手动运行。
...
**一键运行：**进入 **Actions → All Releases (Manual) → Run workflow**，选择默认分支（当前为 `master`）并运行，即可同时执行两个发布子任务。只想执行其中一个时，选择 **Nightly Release** 或 **Stable Release**，同样点击 **Run workflow**。仅检查编译和打包时，选择 **Build Release Package**。自动运行记录在 **Daily Releases (Scheduled)** 中查看，子工作流不单独配置定时器，避免同一天重复触发。

**强制构建并发布：**进入 **Actions → Force Build and Release (Manual) → Run workflow**，选择默认分支（当前为 `master`）、`channel`（`both`、`nightly` 或 `stable`）及 `deployment-mode`。该入口仅手动触发，执行完整的构建、测试和发布流程。正式版子任务接收 `force-release: true`，即使该版本已经发布也会重新构建当前源码。测试成功后替换同版本 ZIP 和 `SHA256SUMS.txt`、清理过时的程序 ZIP、更新发布说明中的构建时间和提交，并将对应标签移动到实际测试通过的源码提交。Release 地址保持不变；判断是否更新请查看附件上传时间及 **Built at / 构建时间**，Release 本身的原始发布日期是另一项信息。新包同时保留在 **Artifacts** 中 7 天。两个渠道独立执行并沿用正常发布的并发锁。版本检查仍生效：`stable` 要求纯 `X.Y.Z`，预发布版本请选 `nightly`。仅需构建产物时使用 **Build Release Package**。推送工作流修改后，请在更新后的默认分支发起新的运行；重跑旧记录仍使用旧记录对应的代码。

两个发布子任务共用构建测试流程和 [发布脚本](.github/scripts/publish-release.sh)：还原依赖、编译 Release、执行控制台回归测试，再生成 Windows x86、x64、ARM64 和可移植 ZIP，以及覆盖全部包的 `SHA256SUMS.txt`。所有包包含语言包和许可证，是否附带运行时由部署模式决定。自动与手动总任务共用 `all-releases` 并发锁，保留正在进行的发布；该锁与子工作流锁不同，避免父子任务相互等待。同一渠道的总任务调用与独立手动运行共用渠道锁，避免同时覆盖相同附件；不同渠道可以并行执行。
...
将工作流提交并推送到 GitHub 默认分支后即可启用，发布使用自动提供的 `GITHUB_TOKEN`，无需配置个人 Token。Fork 需修改两个普通总任务、强制发布入口的校验步骤、预览子任务和正式子任务的仓库名条件。如 Actions 被禁用，请在 **Settings → Actions → General** 启用。滚动预览需要允许 `v*-nightly` 标签更新；仓库 **Settings → General → Releases → Enable release immutability** 应保持关闭，因为该开关会锁定新发布的预览版。脚本遇到不可变 Release 时会报错并保留原版本，不会删除重建。参见 [GitHub 不可变发布说明](https://docs.github.com/en/code-security/concepts/supply-chain-security/immutable-releases)。

修改定时时间请调整 `daily-release.yml` 中的 UTC cron（当前 `0 16 * * *`），对应每天北京时间 **00:00**。这是计划触发时间，GitHub 不保证准点开始执行，整点高峰尤其可能延迟。定时工作流只从默认分支运行；公开仓库连续 60 天无活动时会自动停用，可在 Actions 页面重新启用。参见 [GitHub 定时工作流说明](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule)。
