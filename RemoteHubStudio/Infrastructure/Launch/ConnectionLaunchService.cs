...
public sealed class ConnectionLaunchService
{
    ...
    private readonly ToDeskClientStartup _toDeskStartup;
    private readonly TimeSpan _launchTimeout;
    private readonly TimeProvider _timeProvider;
    private readonly Func<LaunchPlan, Process> _startProcess;
    private readonly Func<ConnectionProfile, AppSettings, LaunchPlan> _planFactory;
    ...
    public ConnectionLaunchService(string rdpDirectory)
        : this(rdpDirectory, ConnectionLaunchOptions.FromEnvironment())
    {
    }

    private ConnectionLaunchService(string rdpDirectory, ConnectionLaunchOptions options)
        : this(rdpDirectory, ToDeskClientStartup.CreateDefault(options.Timeout), options.Timeout)
    {
    }

    /// <summary>Initializes a service with a testable ToDesk startup coordinator. / 使用可测试的 ToDesk 启动协调器初始化服务。</summary>
    internal ConnectionLaunchService(
        string rdpDirectory,
        ToDeskClientStartup toDeskStartup,
        TimeSpan? launchTimeout = null,
        Func<LaunchPlan, Process>? startProcess = null,
        Func<ConnectionProfile, AppSettings, LaunchPlan>? planFactory = null,
        TimeProvider? timeProvider = null)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(rdpDirectory);
        ArgumentNullException.ThrowIfNull(toDeskStartup);
        _rdpDirectory = Path.GetFullPath(rdpDirectory);
        _toDeskStartup = toDeskStartup;
        _launchTimeout = launchTimeout ?? ConnectionLaunchOptions.FromEnvironment().Timeout;
        if (_launchTimeout <= TimeSpan.Zero || _launchTimeout.TotalMilliseconds > uint.MaxValue - 1)
        {
            throw new ArgumentOutOfRangeException(nameof(launchTimeout));
        }
        _startProcess = startProcess ?? StartProcess;
        _planFactory = planFactory ?? CreatePlan;
    ...
    /// <summary>
    /// Creates and starts a connection launch plan after any required client startup. / 创建连接计划，并在所需客户端启动完成后执行。
    /// </summary>
    /// <param name="profile">Connection profile to launch. / 要启动的连接配置。</param>
    /// <param name="settings">Current application settings. / 当前应用设置。</param>
    ...
    public Process Launch(
        ...
        AppSettings settings)
    {
        return LaunchAsync(profile, settings).GetAwaiter().GetResult();
    }

    /// <summary>Starts a connection after asynchronously preparing its client. / 异步等待客户端准备完成后启动连接。</summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(profile);
        ArgumentNullException.ThrowIfNull(settings);
        return RunLaunchAsync(() => _planFactory(profile, settings), cancellationToken);
    }
    ...
    {
        return StartAsync(plan).GetAwaiter().GetResult();
    }

    /// <summary>Waits for ToDesk when required, then starts the connection command. / 按需等待 ToDesk 就绪，再启动连接命令。</summary>
    public Task<Process> StartAsync(LaunchPlan plan, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(plan);
        return RunLaunchAsync(() => plan, cancellationToken, plan);
    }

    /// <summary>Bounds all launch stages without waiting for an uninterruptible native call to return. / 对全部启动阶段设置截止时间，不等待无法中断的原生调用返回。</summary>
    private Task<Process> RunLaunchAsync(
        Func<LaunchPlan> createPlan,
        CancellationToken cancellationToken,
        LaunchPlan? existingPlan = null)
    {
        // Start the deadline before queuing work, including time spent waiting for a worker. Tests can
        // advance an injected clock after reaching a blocked stage; production uses the system clock.
        // / 截止时间包含后台任务的排队时间。测试到达阻塞阶段后再推进注入的时钟，实际程序使用系统时钟。
        CancellationTokenSource timeout = new(_launchTimeout, _timeProvider);
        CancellationTokenSource deadline = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken, timeout.Token);
        CancellationToken operationToken = deadline.Token;
        TaskCompletionSource<Process> completion = new(TaskCreationOptions.RunContinuationsAsynchronously);

        // Cancellation completes the caller independently of the worker. Process.Start and file-system calls
        // cannot always be interrupted; they must never keep the main window or application exit waiting.
        // / 取消会独立完成调用方的任务。Process.Start 和文件系统调用并非总能中断，不能让主窗口或退出继续等待它们。
        void CompleteCancellation()
        {
            ...
            else
            {
                completion.TrySetException(new TimeoutException(
                    $"Connection startup timed out after {_launchTimeout.TotalSeconds.ToString("0.###", CultureInfo.InvariantCulture)} seconds. / 连接启动超时（{_launchTimeout.TotalSeconds.ToString("0.###", CultureInfo.InvariantCulture)} 秒）。"));
            }
        }

        void ThrowIfLaunchCanceled()
        {
            // CancelAsync marks the caller's token before linked callbacks run. Check both tokens so
            // a delayed callback cannot allow a stopped launch to resume or complete successfully.
            // / CancelAsync 先标记调用方令牌，再执行联动回调。同时检查两个令牌，防止回调延迟导致已停止的连接继续启动。
            cancellationToken.ThrowIfCancellationRequested();
            operationToken.ThrowIfCancellationRequested();
        }

        CancellationTokenRegistration cancellationRegistration = operationToken.Register(CompleteCancellation);

        _ = Task.Run(async () =>
        {
            LaunchPlan? plan = existingPlan;
            Process? process = null;
            bool cleanupRegistered = false;
            try
            {
                ThrowIfLaunchCanceled();
                plan ??= createPlan();
                ThrowIfLaunchCanceled();
                if (plan.RequiresToDeskStartup)
                {
                    await _toDeskStartup.EnsureRunningAsync(plan.ExecutablePath, operationToken).ConfigureAwait(false);
                }

                ThrowIfLaunchCanceled();
                process = _startProcess(plan);
                ProcessCleanupRegistration cleanup = new(process, plan.TemporaryFiles);
                cleanupRegistered = true;
                cleanup.CleanupIfAlreadyExited();
                ThrowIfLaunchCanceled();

                // Exactly one result wins: successful handoff OR stop/timeout. A process returned after
                // cancellation belongs to this attempt only; never terminate other clients by process name.
                // / 成功交接与停止/超时只能有一个生效。取消后才返回的进程仅属于本次尝试，绝不按进程名关闭其他客户端。
                if (!completion.TrySetResult(process))
                {
                    StopUnclaimedProcess(process);
                }
            }
            ...
            finally
            {
                ...
                {
                    process?.Dispose();
                    if (plan is not null) DeleteTemporaryFiles(plan.TemporaryFiles);
                }

                // These resources stay with the worker if the caller stops waiting before it unwinds.
                // / 调用方提前停止等待时，这些资源仍由后台任务持有，直至它实际结束。
    ...
    /// <summary>Stops only a newly launched process whose attempt was already canceled or timed out. / 仅停止所属启动尝试已取消或超时的新进程。</summary>
    private static void StopUnclaimedProcess(Process process)
    {
        try
        {
        ...
        {
            // Exit cleanup owns its handle and artifacts, even if the process exited during cancellation.
            // / 即使进程在取消期间退出，仍由退出清理组件管理其句柄与临时文件。
        }
    }

    /// <summary>Starts one process; the launch worker owns result handoff and exit cleanup. / 启动单个进程；启动工作任务负责结果交接与退出清理。</summary>
    private static Process StartProcess(LaunchPlan plan)
    {
        Process process = new()
        {
        ...
        bool allowPassword)
    {
        ...
        return new LaunchPlan(executablePath, arguments, containsSensitiveData: containsSensitiveData,
            requiresToDeskStartup: profile.Type == ConnectionType.ToDesk);
    }
