...
public sealed class LaunchPlan
{
    ...
    /// <param name="requiresToDeskStartup">Whether ToDesk must be running before sending these arguments. / 发送这些参数前是否需要先启动 ToDesk。</param>
    public LaunchPlan(
        string executablePath,
        IEnumerable<string> arguments,
        IEnumerable<string>? temporaryFiles = null,
        bool containsSensitiveData = false,
        bool requiresToDeskStartup = false)
    {
        ...
        RequiresToDeskStartup = requiresToDeskStartup;
    }
    ...
    /// <summary>Gets whether ToDesk startup must complete before this command. / 获取此命令是否需要先等待 ToDesk 启动完成。</summary>
    public bool RequiresToDeskStartup { get; }
