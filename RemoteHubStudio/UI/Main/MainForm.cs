...
public sealed partial class MainForm : AntdUI.Window
{
    ...
    private readonly WindowPlacementStore _windowPlacementStore = null!;
    ...
    private AntdUI.Button _stopConnectionButton = null!;
    ...
    private AntdUI.Button _resetWindowButton = null!;
    ...
    // One source owns the whole launch, regardless of client type or entry point.
    // / 不论客户端类型或入口，每次连接共用一个取消源，覆盖整个启动流程。
    private CancellationTokenSource? _launchCancellation;
    private Task _launchCancellationCallbacks = Task.CompletedTask;
    private string? _launchingConnectionName;
    ...
    private Rectangle? _defaultWindowBounds;
    private Rectangle? _pendingInitialWindowBounds;
    private bool _applyingResetBounds;
    private bool _resetWindowInProgress;
        ...
        SingleInstanceCoordinator singleInstance)
    {
        ...
        _windowPlacementStore = new WindowPlacementStore(_paths);
    ...
    private void ApplyLocalizedToolTips()
    {
        ...
        _toolTip.SetToolTip(_resetWindowButton, L.Get("Main.ToolTip.ResetWindow"));
        ...
        _toolTip.SetToolTip(_stopConnectionButton, L.Get("Main.ToolTip.StopConnection"));
    }
    ...
    private void WireEvents()
    {
        ...
        _stopConnectionButton.Click += HandleToolbarClick;
        ...
        _resetWindowButton.Click += HandleResetWindowClick;
    ...
    private void UpdateActionState()
    {
        ...
        _stopConnectionButton.Enabled = _launchCancellation is { IsCancellationRequested: false };
        ...
        string expiringFilterStatus = _expiringFilterButton.Toggle
            ...
            : string.Empty;
        _viewStatus.Text = _launchCancellation is { } launchCancellation
            ? launchCancellation.IsCancellationRequested
                ? L.Get("Main.Message.StoppingConnection")
                : L.Format("Main.Message.WaitingForConnection", _launchingConnectionName)
            : L.Format(
    ...
    private void ApplyResponsiveLayout()
    {
        ...
        if (!_runtimeInitialized || _applyingResetBounds || !Visible || WindowState == FormWindowState.Minimized)
        {
            return;
        }

        float scale = DeviceDpi <= 0 ? 1F : DeviceDpi / 96F;
        Size layoutClientSize = GetLayoutClientSize();
        float logicalWidth = layoutClientSize.Width / scale;
        float logicalHeight = layoutClientSize.Height / scale;
        ApplyResponsiveMinimumSize();
        bool collapsedSidebar = _workspace.GetSettings().SidebarCollapsed;
        _navigation.SetCollapsed(collapsedSidebar);
        _sidebar.Width = ScaleLogical(MainResponsiveLayoutLogic.CalculateSidebarWidth(logicalWidth, collapsedSidebar));
        _navigation.InlineIndent = logicalWidth < 1080F ? 12 : 20;
        _groupsButton.Text = collapsedSidebar
            ? L.Get("Main.Command.Groups.Compact")
            : L.Get("Main.Command.Groups");
        _groupsButton.Width = ScaleLogical(collapsedSidebar ? 52 : 102);
        _header.SubText = logicalWidth < 1080F
            ? string.Empty
            : L.Get("Main.Header.Subtitle");
        bool iconOnlyReset = logicalWidth < 600F;
        _resetWindowButton.Text = iconOnlyReset ? string.Empty : L.Get("MainForm._resetWindowButton.Text");
        _resetWindowButton.Size = new Size(ScaleLogical(iconOnlyReset ? 36 : 128), ScaleLogical(36));
        _resetWindowButton.Location = new Point(
            Math.Max(ScaleLogical(8), _header.ClientSize.Width - ScaleLogical(iconOnlyReset ? 306 : 398)),
            ScaleLogical(11));
        ...
        int toolbarAvailableWidth = Math.Max(1, layoutClientSize.Width - _sidebar.Width - ScaleLogical(32));
        ...
        ApplyToolbarLabelMode(layoutPlan.CompactToolbarText, logicalToolbarWidth < 380F);
        ApplyToolbarOverflowMode(layoutPlan.UseToolbarOverflow);
        RestorePrimaryToolbarOrder();
        ...
        // Localized labels can fit in fewer rows than the conservative breakpoint estimate.
        // / 本地化按钮可能比断点的保守估计少占一行，将空余高度留给连接列表。
        _toolbar.Height = Math.Max(ScaleLogical(56), measuredToolbarHeight);
        ...
        bool showSecondaryColumns = logicalToolbarWidth >= 800F;
    ...
    /// <summary>Reads the custom-chrome window's live client size for consistent toolbar measurements. / 读取自绘边框窗口的实时客户区尺寸，确保工具栏测量一致。</summary>
    private Size GetLayoutClientSize()
    {
        // WinForms can cache the requested ClientSize including a native frame adjustment while
        // AntdUI removes that frame. Tray handle recreation then corrects the cache by 16 x 39 px
        // at 96 DPI without actually resizing any docked controls. Measure the native client so
        // restoring from the tray cannot shift the pending connection's Stop button.
        // / AntdUI 移除了原生边框，但 WinForms 的 ClientSize 缓存可能仍保留边框补偿；
        // 托盘恢复重建句柄时，缓存会在 96 DPI 下修正 16 x 39 像素，实际停靠控件并未改变大小。
        // 使用实时客户区避免恢复窗口时，等待中的“停止连接”按钮发生位置偏移。
        if (IsHandleCreated && GetClientRect(Handle, out NativeClientRectangle rectangle))
        {
    ...
    private struct NativeClientRectangle
    {
        public int Left;
        public int Top;
        public int Right;
    ...
    private static extern bool GetClientRect(IntPtr windowHandle, out NativeClientRectangle rectangle);

    /// <summary>Preserves Connect/Stop placement when tray restore recreates hidden controls' handles. / 托盘恢复重建隐藏控件句柄时，保持连接与停止按钮的位置。</summary>
    private void RestorePrimaryToolbarOrder()
    {
        Control[] controls = [_searchInput, _typeFilter, _toolbarSpacer, _addButton,
            _connectButton, _stopConnectionButton, _editButton, _deleteButton];
        for (int index = 0; index < controls.Length; index++)
        {
            if (_toolbar.Controls.GetChildIndex(controls[index]) != index)
            {
                _toolbar.Controls.SetChildIndex(controls[index], index);
            }
        }
    }
    ...
    /// <param name="iconOnlyPrimaryCommands">Whether primary actions use icons to leave room for named groups. / 主要操作是否仅显示图标，为分组名称保留空间。</param>
    private void ApplyToolbarLabelMode(bool compact, bool iconOnlyPrimaryCommands)
    {
        ...
        // Keep the full stop label until the other primary commands become icons.
        // / 只有其他主命令已缩为图标时才缩短停止文案，便于直接找到“停止连接”。
        _stopConnectionButton.Text = L.Get(iconOnlyPrimaryCommands ? "Main.Command.StopConnection.Compact" : "Main.Command.StopConnection");
        ...
        _stopConnectionButton.AccessibleName = L.Get("Main.Command.StopConnection");
        ...
        if (iconOnlyPrimaryCommands)
        {
            foreach (AntdUI.Button button in new[] { _addButton, _connectButton, _editButton, _deleteButton })
            {
    ...
    private void ApplyResponsiveMinimumSize()
    {
        ...
        int verticalMargin = ScaleLogical(16);
    ...
    private void ApplyInitialWindowBounds(Rectangle bounds)
    {
        ...
        Rectangle initialBounds = MainResponsiveLayoutLogic.ClampWindowBounds(
            bounds,
            workingArea,
            MinimumSize,
            new Size(1280, 800),
            ScaleLogical(8));
        _pendingInitialWindowBounds = initialBounds;
        ScreenRectangle = initialBounds;
    }

    /// <summary>Applies physical window bounds without intermediate layout or AntdUI size-cache interference. / 应用物理窗口边界，避免中间布局和 AntdUI 尺寸缓存干扰。</summary>
    private void ApplyPhysicalWindowBounds(Rectangle bounds)
    {
        bool wasApplyingBounds = _applyingResetBounds;
        _applyingResetBounds = true;
        try
        {
            MinimumSize = Size.Empty;
            // An equal Location assignment clears AntdUI's normal-size cache without moving.
            // / 同位置赋值可清除 AntdUI 的普通窗口尺寸缓存而不移动窗口。
        ...
        {
            _applyingResetBounds = wasApplyingBounds;
        }
    }
    ...
    {
        if (_applyingResetBounds || WindowState != FormWindowState.Normal)
        {
            return;
        }

        Rectangle workingArea = Screen.FromControl(this).WorkingArea;
        ScreenRectangle = MainResponsiveLayoutLogic.ClampWindowBounds(
            Bounds,
            workingArea,
            MinimumSize,
    ...
    /// <summary>
    /// Restores the placement computed at startup, independent of the current window geometry. / 恢复启动时计算的位置与尺寸，不受当前窗口几何状态影响。
    /// </summary>
    private async void HandleResetWindowClick(object? sender, EventArgs e)
    {
        if (!CanUpdateUi() || _resetWindowInProgress || _defaultWindowBounds is not Rectangle targetBounds) return;

        // Only replace the startup baseline if the display configuration makes it unreachable.
        // Ordinary dragging, resizing and repeated resets must never redefine it.
        // / 仅当显示器配置变化使启动基准不可达时替换；拖动、缩放和重复重置都不改变它。
        if (!Screen.AllScreens.Any(screen => screen.WorkingArea.Contains(targetBounds)))
        {
            targetBounds = MainResponsiveLayoutLogic.CreateDefaultWindowBounds(Screen.FromControl(this).WorkingArea, DeviceDpi);
            _defaultWindowBounds = targetBounds;
            _windowPlacementStore.SaveDefaultBounds(targetBounds);
        }

        _resetWindowInProgress = true;
        _resetWindowButton.Enabled = false;
        try
        {
            _applyingResetBounds = true;
            try
            {
                ...
                ApplyPhysicalWindowBounds(targetBounds);
            }
            finally
            {
                _applyingResetBounds = false;
            }

            ApplyResponsiveLayout();
            await SaveWindowBoundsAsync();
        }
        finally
        {
            _resetWindowInProgress = false;
            if (CanUpdateUi()) _resetWindowButton.Enabled = true;
        }
    }
    ...
    private async void HandleConnectionCellDoubleClick(object sender, AntdUI.TableClickEventArgs e)
    {
            ...
            !string.Equals(e.Column?.Key, nameof(ConnectionTableRow.Favorite), StringComparison.Ordinal))
        {
            ...
            await RunPrimaryOperationAsync(LaunchSelectedConnectionAsync, L.Get("Error.UnableToLaunch"));
        }
    }
    ...
    private async void HandleToolbarClick(object? sender, EventArgs e)
    {
        ...
        if (!CanUpdateUi() || _primaryOperationInFlight && command is not ("check" or "stop-connection"))
        {
            return;
        }
        ...
        try
        {
            switch (command)
            {
                case "add":
                    ...
                    break;
                case "quick":
                    await RunPrimaryOperationAsync(QuickConnectAsync, L.Get("Error.UnableToLaunch"));
                    break;
                case "connect":
                    await RunPrimaryOperationAsync(LaunchSelectedConnectionAsync, L.Get("Error.UnableToLaunch"));
                    break;
                case "stop-connection":
                    CancelPendingConnectionLaunch();
                    break;
                case "edit":
    ...
    /// <summary>
    /// Enables or disables commands that could conflict with a primary launch, save, import, export, or modal edit. / 启用或禁用可能与主要启动、保存、导入、导出或模态编辑冲突的命令。
    /// </summary>
    ...
    private void SetPrimaryOperationState(bool busy)
    {
        ...
        // Stop remains reachable while the primary-operation guard disables conflicting commands.
        // / 主要操作锁禁用冲突命令时，停止按钮必须仍可使用。
        _stopConnectionButton.Enabled = _launchCancellation is { IsCancellationRequested: false };
    ...
    private async Task QuickConnectAsync()
    {
        ...
        {
            await LaunchProfileAsync(profile);
        }
    }
    ...
    /// </summary>
    private async Task LaunchSelectedConnectionAsync()
    {
    ...
    /// <summary>
    /// Awaits client readiness and starts a connection using its inline authentication values and current security settings. / 等待客户端就绪后，使用连接内联认证信息与当前安全设置启动连接。
    /// </summary>
    /// <param name="profile">Connection profile to launch. / 要启动的连接配置。</param>
    private async Task LaunchProfileAsync(ConnectionProfile profile)
    {
        CancellationTokenSource launchCancellation = new();
        _launchCancellation = launchCancellation;
        _launchCancellationCallbacks = Task.CompletedTask;
        _launchingConnectionName = profile.Name;
        try
        {
            SetConnectionLaunchState(launchInProgress: true);
            if (profile.Type == ConnectionType.ToDesk)
            {
                AntdUI.Message.info(this, L.Get("Main.Message.WaitingForToDesk"));
            }

            await _launchService.LaunchAsync(profile, _workspace.GetSettings(), launchCancellation.Token);
            // A canceled request must never show a late success, even if an external launcher returned.
            // / 即使外部启动器此时返回，已取消的请求也不能再显示迟到的成功提示。
            launchCancellation.Token.ThrowIfCancellationRequested();
            if (CanUpdateUi() && ReferenceEquals(_launchCancellation, launchCancellation))
            {
        ...
        catch (OperationCanceledException) when (launchCancellation.IsCancellationRequested)
        {
            if (CanUpdateUi() && ReferenceEquals(_launchCancellation, launchCancellation))
            {
                AntdUI.Message.info(this, L.Get("Main.Message.ConnectionStopped"));
            }
        }
        catch (TimeoutException exception)
        {
            ...
            {
                ShowError(L.Get("Main.Message.ConnectionTimedOut"), exception);
            }
        }
        finally
        {
            Task cancellationCallbacks = _launchCancellationCallbacks;
            if (ReferenceEquals(_launchCancellation, launchCancellation))
            {
                _launchCancellation = null;
                _launchCancellationCallbacks = Task.CompletedTask;
                _launchingConnectionName = null;
                if (CanUpdateUi())
                {
                    SetConnectionLaunchState(launchInProgress: false);
                }
            }

            // Callback cleanup must not keep the UI operation guard or closing window waiting.
            // / 取消回调清理不能继续占用 UI 操作锁，也不能阻塞关闭窗口。
            _ = DisposeLaunchCancellationAsync(launchCancellation, cancellationCallbacks);
        }
    }

    /// <summary>Replaces Connect with a visible Stop action for every pending client launch. / 所有客户端启动期间，将连接按钮替换为可见的停止操作。</summary>
    private void SetConnectionLaunchState(bool launchInProgress)
    {
        // The primary toolbar is never moved into the disabled overflow menu on narrow screens.
        // / 主工具栏在窄屏时不会移入已禁用的溢出菜单，确保停止入口始终可见。
        _connectButton.Visible = !launchInProgress;
        _stopConnectionButton.Visible = launchInProgress;
        _connectButton.Loading = launchInProgress;
        UpdateActionState();
        ApplyResponsiveLayout();
    }

    /// <summary>Signals cancellation without running potentially slow callbacks on the UI thread. / 发出取消信号，避免在 UI 线程执行可能缓慢的取消回调。</summary>
    private void CancelPendingConnectionLaunch()
    {
        if (_launchCancellation is not { IsCancellationRequested: false } launchCancellation)
        {
            return;
        }

        _launchCancellationCallbacks = launchCancellation.CancelAsync();
        if (CanUpdateUi())
        {
            UpdateActionState();
        }
    ...
    /// <summary>Lets the owning launch dispose its source after asynchronous callbacks complete. / 由所属连接在异步回调完成后释放取消源。</summary>
    private static async Task DisposeLaunchCancellationAsync(CancellationTokenSource source, Task callbacks)
    {
        try
        {
        ...
        {
            Debug.WriteLine($"Launch cancellation callback failed: {exception.GetType().Name}");
        }
        finally
        {
            source.Dispose();
    ...
    /// <summary>
    /// Restores saved physical bounds after AntdUI has removed the native non-client frame. / AntdUI 移除系统非客户区边框后恢复保存的物理边界。
    /// </summary>
    protected override void OnLoad(EventArgs e)
    {
        base.OnLoad(e);
        if (!_runtimeInitialized || _pendingInitialWindowBounds is not Rectangle bounds) return;

        _pendingInitialWindowBounds = null;
        ApplyResponsiveMinimumSize();
        Rectangle restoredBounds = MainResponsiveLayoutLogic.ClampWindowBounds(
            bounds,
            Screen.FromRectangle(bounds).WorkingArea,
            MinimumSize,
            new Size(ScaleLogical(1280), ScaleLogical(800)),
            ScaleLogical(8));
        ApplyPhysicalWindowBounds(restoredBounds);
    }
    ...
    protected override void OnShown(EventArgs e)
    {
        ...
        if (_defaultWindowBounds is null)
        {
            // Keep reset placement independent of the last window bounds across process launches.
            // / 跨进程启动保存独立的重置基准，不随最近一次窗口边界变化。
            Rectangle? savedDefault = _windowPlacementStore.LoadDefaultBounds();
            if (savedDefault is Rectangle saved && Screen.AllScreens.Any(screen => screen.WorkingArea.Contains(saved)))
            {
                _defaultWindowBounds = saved;
            }
            else
            {
                Rectangle preferred = MainResponsiveLayoutLogic.CreateDefaultWindowBounds(Screen.FromControl(this).WorkingArea, DeviceDpi);
                _defaultWindowBounds = preferred;
                _windowPlacementStore.SaveDefaultBounds(preferred);
            }
        }
    ...
    /// <summary>Persists a normal window after either dragging or resetting it. / 拖动或重置后保存普通窗口边界。</summary>
    private async Task SaveWindowBoundsAsync()
    {
        if (_applyingResetBounds || WindowState != FormWindowState.Normal)
        {
            return;
    ...
    private void HandleFormClosing(object? sender, FormClosingEventArgs e)
    {
        ...
        // A pending external client must not veto a real exit. Keep unrelated writes protected.
        // / 等待外部客户端不能阻止真正退出；与连接无关的保存仍需安全完成。
        bool pendingLaunch = _launchCancellation is not null;
        CancelPendingConnectionLaunch();
        if (((_primaryOperationInFlight && !pendingLaunch) || _windowBoundsSaveCount > 0) &&
