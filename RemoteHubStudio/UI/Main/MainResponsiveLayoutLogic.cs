...
public static class MainResponsiveLayoutLogic
{
    ...
    /// <summary>
    /// Keeps named groups visible as the sidebar narrows; only an explicit preference hides labels. / 缩窄侧栏时保留分组名称，仅显式折叠设置隐藏文字。
    /// </summary>
    public static int CalculateSidebarWidth(float logicalClientWidth, bool collapsed)
    {
        if (!float.IsFinite(logicalClientWidth) || logicalClientWidth < 0F)
        {
            throw new ArgumentOutOfRangeException(nameof(logicalClientWidth));
        }

        return collapsed ? 72 : logicalClientWidth >= 1080F ? 232 : logicalClientWidth >= 900F ? 200 : logicalClientWidth >= 720F ? 184 : 168;
    }

    /// <summary>
    /// Centers the default 1280 × 800 logical window inside the current monitor's usable area. / 将默认 1280 × 800 逻辑尺寸的窗口居中放入当前显示器可用区域。
    /// </summary>
    public static Rectangle CreateDefaultWindowBounds(Rectangle workingArea, int deviceDpi)
    {
        ...
        return ClampWindowBounds(
            Rectangle.Empty,
            workingArea,
            new Size(Scale(720), Scale(480)),
            new Size(Scale(1280), Scale(800)),
            Scale(8));
    }
    ...
    public static MainResponsiveLayoutPlan CreatePlan(float logicalToolbarWidth, float logicalHeight)
    {
        ...
        if (logicalToolbarWidth is >= 280F and < 380F)
        {
            // Fit both filters on one row; icon-only primary commands occupy the next row.
            // / 两个筛选框共用一行，主要操作图标占用下一行。
            searchWidth = Math.Min(searchWidth, (int)logicalToolbarWidth - typeFilterWidth - 16);
        }
