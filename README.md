...
Download a stable or daily preview ZIP from [Releases](https://github.com/1609676823/RemoteHubStudio/releases); versions containing `nightly` are previews. Most users should select the **self-contained** `win-x86`, `win-x64`, or `win-arm64` package for their CPU, extract all files, and run `RemoteHubStudio.exe` without installing .NET separately. **Framework-dependent** and **win-portable** packages require the .NET 10 Windows Desktop Runtime. Portable packages also contain `RemoteHubStudio.exe` (x64 on current Actions runners; requires x64 Desktop Runtime) and retain `Start-RemoteHubStudio.cmd` / `dotnet RemoteHubStudio.dll` to run the AnyCPU DLL with another architecture's dotnet and matching Desktop Runtime. See [deployment targets and modes](docs/DEPLOYMENT.md).
...
### Stopping connections and startup timeout

Every client type shows a **Stop connection** button while launching. It cancels the pending launch/wait and restores the controls. A real application exit also cancels the wait. The overall timeout covers argument preparation, client readiness (including ToDesk), and process startup, and defaults to **15 seconds**. Sessions already handed to external clients are managed by those clients; this button stops a pending launch.

Set the environment variable before starting a fresh app process to change the timeout (whole seconds, `1–3600`; missing, invalid, or out-of-range values fall back to 15 seconds):
...
The setting is read at application startup. Exit any running instance before restarting with a new value. Defaults, bounds, parsing, and maintenance comments are in `RemoteHubStudio/Infrastructure/Launch/ConnectionLaunchOptions.cs`. ToDesk's 10-second initialization grace and 3-second stability interval are tuned in `ToDeskClientStartup.CreateDefault`, within the same overall deadline.

### ToDesk notes

Before connecting, RemoteHubStudio checks for the configured ToDesk executable in the current Windows session and opens it without arguments if absent. Once the client has been running for at least 10 seconds, every window thread must respond continuously for 3 seconds before the connection command is sent. A stall, process replacement, or window change restarts this stability interval. Existing clients are reused. The main window stays responsive and shows a waiting notice; the request follows the overall deadline above and continues automatically after recovery, without another click. Stopping, startup failure, or persistent unresponsiveness ends the wait without sending the connection command. Custom ToDesk arguments use the same checks. Window responsiveness does not confirm a successful remote connection.
...
The [Daily Releases (Scheduled) workflow](.github/workflows/daily-release.yml) is the only scheduled entry point; [All Releases (Manual)](.github/workflows/all-releases.yml) is the separate manual entry point. This follows GitHub's [reusable workflow pattern](https://docs.github.com/en/actions/how-tos/reuse-automations/reuse-workflows): keep trigger-specific settings in separate entry points and share the build and publication logic through `workflow_call`. Automatic and manual runs have separate histories in Actions. Each parent calls the preview and stable child workflows **in parallel**, with both children and their build, test, and publication steps visible in that parent's run. Neither child depends on the other succeeding, and every child retains its standalone manual trigger.
...
**Run everything:** open **Actions → All Releases (Manual) → Run workflow**, select the default branch (currently `master`), and run. To execute just one publication child, choose **Nightly Release** or **Stable Release** and click **Run workflow**. Choose **Build Release Package** to check packaging only. View automatic runs under **Daily Releases (Scheduled)**. Child workflows have no timers of their own, avoiding duplicate scheduled builds.

**Force a rebuild and publication:** open **Actions → Force Build and Release (Manual) → Run workflow**, select the default branch (currently `master`), `channel` (`both`, `nightly`, or `stable`), and `deployment-mode`. This manual-only workflow runs the complete build, test, and publication chain. The stable child receives `force-release: true`, rebuilding the current source even when the version is already published. After tests pass, it replaces that version's ZIPs and `SHA256SUMS.txt`, removes obsolete application ZIPs, updates the release notes with the new build time and commit, and moves the corresponding tag to the tested commit. The Release URL stays the same. Check the attachment upload times and the **Built at** entry; the Release's original publication date is separate. Fresh packages are also available in **Artifacts** for 7 days. Both channels run independently and share their normal publication locks. Version checks still apply: `stable` requires a plain `X.Y.Z`; select `nightly` for prerelease versions. Use **Build Release Package** when only Actions artifacts are needed. After pushing workflow changes, start a new run on the updated default branch; rerunning an old run uses its original revision.

Both publication children share the build and test workflow and [publishing script](.github/scripts/publish-release.sh): restore dependencies, build Release, run the console regression suite, and create Windows x86, x64, ARM64, and portable ZIPs with a combined `SHA256SUMS.txt`. Every package includes language packs and license notices; runtime inclusion depends on deployment mode. The scheduled and manual parents share an `all-releases` concurrency lock and keep in-progress releases running. This lock differs from the child locks to avoid a parent waiting on its own child. Parent-triggered and standalone runs of the same channel share a concurrency lock to prevent overlapping asset replacements; different channels can run concurrently.
...
Commit and push the workflows to GitHub's default branch to enable them. The supplied `GITHUB_TOKEN` handles publication without a personal token. Forks must update the repository conditions in both normal parents, the force-publication validation step, and the nightly and stable workflows. Enable Actions under **Settings → Actions → General** if necessary. Rolling previews require updates to `v*-nightly` tags to be allowed. Keep **Settings → General → Releases → Enable release immutability** disabled for this repository: that setting would lock newly published previews. The script fails without deleting or recreating an immutable release. See [GitHub's immutable release documentation](https://docs.github.com/en/code-security/concepts/supply-chain-security/immutable-releases).

Adjust the UTC cron expression (`0 16 * * *`) in `daily-release.yml` to change the schedule. It schedules a run for **00:00 Asia/Shanghai** each day; GitHub does not guarantee an exact start time and may delay runs, especially at the start of an hour. Scheduled workflows only run from the default branch and may be disabled after 60 days without public repository activity; re-enable them from Actions when needed. See [GitHub's schedule documentation](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule).
